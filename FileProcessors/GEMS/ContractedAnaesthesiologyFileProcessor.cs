...
public sealed class ContractedAnaesthesiologyFileProcessor(
...
{
    private const string DisciplineCode = "10";
    ...
    public async Task ProcessAsync(ProcessFileParameters parameters)
    {
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            foreach (var row in sheet.Rows())
            {
                ...
                if (row.Cell("A").IsEmpty())
                    continue;

                if (!row.Cell("A").IsEmpty() && double.TryParse(row.Cell("A").GetString(), out _))
                {
                    continue;
                }
